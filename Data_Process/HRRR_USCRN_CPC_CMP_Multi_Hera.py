...
from Read_USCRN import read_USCRN
...
#        [ism_pjm,ism_ts1,m_lvls,z_lvls,nx,ny,lons,lats,nslvls,an_date] = pickle.load(open( pickpath+filename, "rb" ))
    ...
    if os.path.exists(savepath+'f_scatter_values_'+YYYY+MM+DD+'_'+ism_txt+'_lowqc.p'):
        continue
    ...
    # Read in USCRN data
    data_uscrn = read_USCRN(YYYY+MM+DD)
    # Only include CONUS
    ...
    data_uscrn_v5 = data_uscrn[data_uscrn['SOIL_MOISTURE_5_DAILY']<0.8]
    data_uscrn_v10 = data_uscrn[data_uscrn['SOIL_MOISTURE_10_DAILY']<0.8]
    data_uscrn_v100 = data_uscrn[data_uscrn['SOIL_MOISTURE_100_DAILY']<0.8]
...
#    from mpl_toolkits.basemap import Basemap
    for anl_cho in anls:
        ...
        # ax.set_title('ISM: '+YY+MM+DD)
        # plt.tight_layout()
        # plt.savefig(savepath+'CPCISM_SMap_USCRN'+YYYY+MM+DD+'_'+ism_txt+'.png')
        # plt.close()
        ...
        # plt.grid()
        # plt.tight_layout()
        # plt.savefig(savepath+'CPC_scatter_ISM_HRRR_CMP_'+YYYY+MM+DD+'_'+ism_txt+'.png')
        # plt.close()
        ...
        #plt.grid()
        #plt.tight_layout()
        #plt.savefig(savepath+'f_CPC_scatter_ISM_HRRR_USCRN_CMP_'+YYYY+MM+DD+'_'+ism_txt+'.png')
        #plt.close()
   ...
   # slat = OrderedDict()
    ...
    for i in np.arange(0,len(m_ids)):
        ...
        if usc_v[i] == '5':
           ...
           usc[usc_v[i]] = data_uscrn_v5['SOIL_MOISTURE_'+usc_v[i]+'_DAILY'].values
   
        elif usc_v[i] == '10':
           vsm[v_ids[i]] = griddata(points,values,(data_uscrn_v10['LONGITUDE'].values+360,data_uscrn_v10['LATITUDE'].values))        
           usc[usc_v[i]] = data_uscrn_v10['SOIL_MOISTURE_'+usc_v[i]+'_DAILY'].values

        elif usc_v[i] == '100': 
           vsm[v_ids[i]] = griddata(points,values,(data_uscrn_v100['LONGITUDE'].values+360,data_uscrn_v100['LATITUDE'].values))
           usc[usc_v[i]] = data_uscrn_v100['SOIL_MOISTURE_'+usc_v[i]+'_DAILY'].values
 
    pickle.dump([usc,vsm,ism_usc,hrrr_int,cpc_int,a_date],open( savepath+'f_scatter_values_'+YYYY+MM+DD+'_'+ism_txt+'_lowqc.p', "wb" ))
