...
savepath = '/scratch1/RDARCH/rda-ghpcs/Peter.Marinescu/python/save/Maps/Mean_STD/'
avg = OrderedDict()
std = OrderedDict()
...
fs = 8; plt.rcParams.update({'font.size': fs}) # Fontsize
ax = OrderedDict()
#fig = plt.figure(figsize=[7.5,28])
fig = plt.figure(figsize=[15,5])
spec = gridspec.GridSpec(ncols=3, nrows=2, figure=fig, wspace=0.05, hspace=0.05,
                         left = 0.03, right = 0.97, top = 0.97, bottom = 0.03 )
gx = 0; gy = 0;

title_l = ['(a) HRRR ISM Mean (mm)','(b) CPC ISM Mean (mm)','(d) HRRR ISM STD (mm)','(e) CPC ISM STD (mm)','(c) HRRR-CPC Difference in ISM Mean (mm)','(f) HRRR-CPC Difference in ISM STD (mm)']
...
#        ax[i].scatter(df_sc['lon'].values,df_sc['lat'].values,c=df_sc['insitu'].values,s=4,marker='o',linewidths=0.1,edgecolors='w',cmap=cmaps[j],vmin=np.min(clvls[j]),vmax=np.max(clvls[j]))
        if j == 0:
            ax[i].scatter(df_s['lon'].values,df_s['lat'].values,c=df_s['insitu'].values,s=7,marker='o',linewidths=0.7,edgecolors='k',cmap=cmaps[j],vmin=np.min(clvls[j]),vmax=np.max(clvls[j]))
        elif j == 1:
            ax[i].scatter(df_s['lon'].values,df_s['lat'].values,c=df_std['insitu'].values,s=7,marker='o',linewidths=0.7,edgecolors='k',cmap=cmaps[j],vmin=np.min(clvls[j]),vmax=np.max(clvls[j]))
...
for j in np.arange(0,2):
        if j == 0:
            varp = avg
            clvls = np.arange(-300,301,30)
        elif j == 1:
            varp = std
            clvls = np.arange(-60,61,6)
        ax[i] = plt.subplot(spec[j,2], projection=ccrs.PlateCarree())
        ax[i].add_feature(states_provinces)
        ax[i].add_feature(coastline)
        ax[i].add_feature(land)
        ax[i].add_feature(country)
        ...
        a = ax[i].contourf(lons, lats, varp[0]-varp[1], levels=clvls, cmap=plt.cm.bwr,vmin=np.min(clvls),vmax=np.max(clvls),extend='both', transform=ccrs.PlateCarree())
        plt.colorbar(a,ax=ax[i])
#        ax[i].scatter(df_su['lon'].values,df_su['lat'].values,c='k',s=3,marker='o')
#        ax[i].scatter(df_sc['lon'].values,df_sc['lat'].values,c='k',s=3,marker='x')
        #ax[i].scatter(df_su['lon'].values,df_su['lat'].values,c=,s=3,marker='o')
        #ax[i].scatter(df_sc['lon'].values,df_sc['lat'].values,c=,s=3,marker='x')
        if j == 0:
            ax[i].scatter(df_s['lon'].values,df_s['lat'].values,c=df_s['hrrr'].values-df_s['insitu'].values,s=7,marker='o',linewidths=0.7,edgecolors='k',cmap=plt.cm.bwr,vmin=np.min(clvls),vmax=np.max(clvls))
        if j == 1:
            ax[i].scatter(df_s['lon'].values,df_s['lat'].values,c=df_std['hrrr'].values-df_std['insitu'].values,s=7,marker='o',linewidths=0.7,edgecolors='k',cmap=plt.cm.bwr,vmin=np.min(clvls),vmax=np.max(clvls))
        ax[i].set_xlim([-128,-65])
        ax[i].set_ylim([24,50])
        ax[i].set_title(title_l[i])
        gl = ax[i].gridlines(crs=ccrs.PlateCarree(), draw_labels=True,
...
plt.tight_layout()
plt.savefig(savepath+'Mean_STD_Maps'+savename2+monsave+'_cbar_sca.png',dpi=600)
plt.savefig(savepath+'Mean_STD_Maps'+savename2+monsave+'_cbar_sca.pdf',dpi=200)
plt.savefig(savepath+'Mean_STD_Maps'+savename2+monsave+'_cbar_sca.eps',dpi=600)
