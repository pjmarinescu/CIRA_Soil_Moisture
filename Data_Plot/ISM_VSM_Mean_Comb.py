...
# Code for plotting ISM and VSM Means on the same plot
# 
"""
...
def get_vsm_mean(start_date,end_date,pv,ism_txt,openpaths,savename2):
    ...
    for j in np.arange(0,len(pcts)-1):
        ...
        for p in np.arange(0,len(pairs)):
            ...
            v1_ip = hrr_ii[p][mean_val[p] >= x_arr[p,j]]
            v2_ip = usc_ii[p][mean_val[p] >= x_arr[p,j]]
            ...
            #Run paired t-test on station quintile data sample
            [tstat[p,j],pval[p,j]] = scipy.stats.ttest_rel(v1_ip,v2_ip,nan_policy='raise')       
    
    return pairs, lon_ii, hrr_ii, usc_ii, mean_val, x_arr, avg_arr, tstat, pval, pcts
    
# Define function to get ISM values from individual date files
def get_ism_mean(start_date,end_date,pv,ism_txt,openpaths,savename2):
    ...
    # Calculate differences or individual days
    df['hmc'] = df['hrrr']-df['cpc']
    df['hmi'] = df['hrrr']-df['insitu']
    df['cmi'] = df['cpc']-df['insitu']
    
    # Group station and calculate mean for each station
    df_stat = df.groupby(['uloc']).mean()    
    print(len(df_stat))
    ...
    df_stat = df2.groupby(['uloc']).mean()
    
    print(len(df_stat))
    
    hrr_ii = df_stat['hrrr'].values
    cpc_ii = df_stat['cpc'].values
    usc_ii = df_stat['insitu'].values
    hmc_ii = df_stat['hmc'].values
    hmi_ii = df_stat['hmi'].values
    cmi_ii = df_stat['cmi'].values
    lon_pp = df_stat['lon'].values
    lat_pp = df_stat['lat'].values
    ...
    avg1_arr = np.zeros((3,len(x_arr)))
    std_arr = np.zeros((3,len(x_arr)))
    cnt_arr = np.zeros((3,len(x_arr)))
    tstat = np.zeros((3,len(x_arr)))
    pval = np.zeros((3,len(x_arr)))
    tstat1 = np.zeros((3,len(x_arr)))
    pval1 = np.zeros((3,len(x_arr)))
    pdif_arr = np.zeros((3,len(x_arr)))
    # Loop through 5 quintiles
    for j in np.arange(0,len(x_arr)-1):
        # Loop through 3 comparisons (H-I, C-I, H-C)
        for i in np.arange(0,3):
            if i == 0: 
                v1_iip = hrr_ii[mean_val[i] >= x_arr[j]]
                v2_iip = usc_ii[mean_val[i] >= x_arr[j]]
                v3_iip = hmi_ii[mean_val[i] >= x_arr[j]]
            elif i == 2:
                v1_iip = hrr_ii[mean_val[i] >= x_arr[j]]
                v2_iip = cpc_ii[mean_val[i] >= x_arr[j]]
                v3_iip = hmc_ii[mean_val[i] >= x_arr[j]]
            elif i == 1:
                v1_iip = cpc_ii[mean_val[i] >= x_arr[j]]
                v2_iip = usc_ii[mean_val[i] >= x_arr[j]]
                v3_iip = cmi_ii[mean_val[i] >= x_arr[j]]
            ...
            pdif_arr[i,j] = np.nanmean((v1_iip-v2_iip)/v2_iip*100)
            avg1_arr[i,j] = np.nanmean(v3_iip)
            std_arr[i,j] = np.nanstd(v1_iip-v2_iip)
            cnt_arr[i,j] = len(v1_iip-v2_iip)
    
    return lon_pp, hrr_ii, usc_ii, cpc_ii, mean_val, x_arr, avg_arr, tstat, pval, pdif_arr
...
[lon_pp, hrr_ii, usc_ii, cpc_ii, mean_val, x_arr, avg_arr, tstat, pval, pdif_arr] = get_ism_mean(start_date,end_date,pv,ism_txt,openpaths,savename2)


# Plotting Specifications
ytix = [-600,-300,0,300,600]
ylims = [-600,600]
ylims2 = [0,240]
ytix2 = [0,60,120,180,240]
xlims = [50,650]
...
ax[2,0].set_xlabel('1.6 m ISM Mean Value (mm)')

for i in np.arange(0,3):
    ax[i,0].set_ylabel('1.6 m ISM \n Mean Difference (mm)')
...
[pairs, lon_ii, hrr_ii, usc_ii, mean_val, x_arr, avg_arr, tstat, pval, pcts] = get_vsm_mean(start_date,end_date,pv,ism_txt,openpaths,savename2)
...
#    b.ax.set_ylabel('Longitude')
    ...
    ax[p,1].set_ylabel('VSM Mean Difference')
...
plt.savefig(savepath+'Scatter_COMBiv_DiffxMeancLon_SCAN_USCRN_PCT20_SIG'+str(1-pv)+savename+savename2+'.png')
plt.savefig(savepath+'Scatter_COMBiv_DiffxMeancLon_SCAN_USCRN_PCT20_SIG'+str(1-pv)+savename+savename2+'.pdf')